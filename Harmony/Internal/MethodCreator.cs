    ...
    internal class MethodCreator
    {
        ...
        internal (MethodInfo, Dictionary<int, CodeInstruction>) CreateReplacement()
        {
            ...
            if (config.AnyFixHas(InjectionType.ResultRef))
            {
                ...
                {
                    var varType = typeof(RefResult<>).MakeGenericType(
                        config.returnType.GetElementType()
                    );
            ...
            config.WithFixes(fix =>
            {
                ...
                foreach (var injection in config.InjectionsFor(fix, InjectionType.State))
                {
                    ...
                    if (maybeLocal != null)
                    {
                        ...
                        {
                            var message =
                                $"__state type mismatch in patch \"{fix.DeclaringType.FullName}.{fix.Name}\": "
            ...
            if (config.skipOriginalLabel.HasValue)
                config.AddCodes([
                    Ldloc[config.runOriginalVariable],
                    Brfalse[config.skipOriginalLabel.Value],
                ]);
            ...
            var replacement = copier.Finalize(
                true,
                out var hasReturnCode,
                out var methodEndsInDeadCode,
                endLabels
            );
            ...
            if (
                config.resultVariable is not null
                && (hasReturnCode || (methodEndsInDeadCode && config.skipOriginalLabel.HasValue))
            )
            ...
            if (
                ...
                || config.skipOriginalLabel is not null
                || config.finalizers.Count > 0
                || config.postfixes.Count > 0
            )
                config.AddCode(Ret);

            config.instructions = MonoILFixup.Apply(config.instructions, config.il);
            ...
            var replacementMethod = config.patch.Generate(config.original.DeclaringType);
        ...
        internal void AddPrefixes()
        {
            foreach (var fix in config.prefixes)
            {
                ...
                if (
                    MethodPatcherTools
                        .OriginalParameters(fix)
                        .Any(pair => pair.realName == MethodPatcherTools.ARGS_ARRAY_VAR)
                )
                ...
                if (refResultUsed)
                {
                    ...
                    config.AddCode(
                        Callvirt[
                            AccessTools.Method(
                                config.GetLocal(InjectionType.ResultRef).LocalType,
                                "Invoke"
                            )
                        ]
                    );
                ...
                tmpBoxVars.Do(tmpBoxVar =>
                {
                    config.AddCode(
                        new CodeInstruction(
                            config.OriginalIsStatic ? OpCodes.Ldarg_0 : OpCodes.Ldarg_1
                        )
                    );
                ...
                if (returnType != typeof(void))
                {
                    if (returnType != typeof(bool))
                        throw new Exception(
                            $"Prefix patch {fix} has not \"bool\" or \"void\" return type: {fix.ReturnType}"
                        );
        ...
        internal bool AddPostfixes(bool passthroughPatches)
        {
            ...
            foreach (
                var fix in config.postfixes.Where(fix =>
                    passthroughPatches == (fix.ReturnType != typeof(void))
                )
            )
            {
                var tmpBoxVars = new List<KeyValuePair<LocalBuilder, Type>>();
                ...
                {
                    config.AddCode(
                        new CodeInstruction(originalIsStatic ? OpCodes.Ldarg_0 : OpCodes.Ldarg_1)
                    );
                    config.AddCode(Ldloc[tmpBoxVar.Key]);
                    config.AddCode(Unbox_Any[tmpBoxVar.Value]);
                    config.AddCode(Stobj[tmpBoxVar.Value]);
                ...
                if (fix.ReturnType != typeof(void))
                {
                    ...
                    var hasPassThroughResultParam =
                        firstFixParam is not null && fix.ReturnType == firstFixParam.ParameterType;
        ...
        IEnumerable<CodeInstruction> AddInfixes(IEnumerable<CodeInstruction> instructions)
        {
            ...
            foreach (var (innerMethod, calls) in callGroups.Select(g => (g.Key, Calls: g.ToList())))
            {
                ...
                for (var i = 0; i < total; i++)
                {
                    ...
                    var prefixes = config
                        .innerprefixes.FilterAndSort(innerMethod, i + 1, total, config.debug)
                        .SelectMany(fix => fix.Apply(config, true));
                    var postfixes = config
                        .innerpostfixes.FilterAndSort(innerMethod, i + 1, total, config.debug)
