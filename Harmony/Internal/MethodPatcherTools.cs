...
namespace HarmonyLib
{
    internal class MethodPatcherTools
    {
        ...
        internal static DynamicMethodDefinition CreateDynamicMethod(
            MethodBase original,
            string suffix,
            bool debug
        )
        {
            ...
            var patchName =
                $"{original.DeclaringType?.FullName ?? "GLOBALTYPE"}.{original.Name}{suffix}";
            ...
            var method = new DynamicMethodDefinition(patchName, returnType, [.. parameterTypes]);
            ...
            for (var i = 0; i < parameters.Length; i++)
            {
                ...
                // HasDefault without a Constant entry crashes Mono while reading parameters.
                param.Attributes &= ~Mono.Cecil.ParameterAttributes.HasDefault;
            ...
            if (debug)
            {
                ...
                FileLog.Log(
                    $"### Replacement: static {returnType.FullDescription()} {original.DeclaringType?.FullName ?? "GLOBALTYPE"}::{patchName}({parameterStrings.Join()})"
                );
            }
        ...
        internal static IEnumerable<(ParameterInfo info, string realName)> OriginalParameters(
            MethodInfo method
        )
        {
            ...
            if (method.DeclaringType is not null)
                baseArgs = baseArgs
                    .Union(method.DeclaringType.GetArgumentAttributes())
        ...
        internal static LocalBuilder[] DeclareOriginalLocalVariables(
            ILGenerator il,
            MethodBase member
        )
        {
        ...
        static readonly MethodInfo m_GetMethodFromHandle1 = typeof(MethodBase).GetMethod(
            "GetMethodFromHandle",
            [typeof(RuntimeMethodHandle)]
        );
        static readonly MethodInfo m_GetMethodFromHandle2 = typeof(MethodBase).GetMethod(
            "GetMethodFromHandle",
        ...
        internal static bool EmitOriginalBaseMethod(MethodBase original, Emitter emitter)
        {
            ...
            emitter.Emit(
                OpCodes.Call,
                type.IsGenericType ? m_GetMethodFromHandle2 : m_GetMethodFromHandle1
            );
            return true;
        }

        static readonly HashSet<Type> PrimitivesWithObjectTypeCode =
        [
            typeof(nint),
            typeof(nuint),
            typeof(IntPtr),
        ...
        internal static OpCode LoadIndOpCodeFor(Type type)
        {
            ...
            return Type.GetTypeCode(type) switch
            {
                ...
                TypeCode.Empty or TypeCode.Object or TypeCode.DBNull or TypeCode.String =>
                    OpCodes.Ldind_Ref,
                _ => OpCodes.Ldind_Ref,
            };
        }
