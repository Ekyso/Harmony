...
namespace MonoMod.Core.Platforms.Runtimes
{
    internal class Core21Runtime : CoreBaseRuntime
    {
        ...
        */
        ...
        private static readonly Func<Core21Runtime, JitHookHelpersHolder> createJitHookHelpersFunc =
            CreateJitHookHelpers;
        ...
        protected unsafe JitHookHelpersHolder JitHookHelpers =>
            Helpers.GetOrInitWithLock(ref lazyJitHookHelpers, sync, createJitHookHelpersFunc, this);
        ...
        protected virtual InvokeCompileMethodPtr InvokeCompileMethodPtr =>
            V21.InvokeCompileMethodPtr;
        ...
        {
            var getVersionIdentPtr = (delegate* unmanaged[Thiscall]<
                IntPtr,
                Guid*,
                void>)ReadObjectVTable(jit, VtableIndexICorJitCompilerGetVersionGuid);
            Guid guid;
            getVersionIdentPtr(jit, &guid);
            Helpers.Assert(
                guid == ExpectedJitVersion,
        ...
        protected override unsafe void InstallManagedJitHook(IntPtr jit)
        {
            ...
            var ourCompileMethodDelegate = CastCompileHookToRealType(
                CreateCompileMethodDelegate(compileMethod)
            );
            ourCompileMethod = ourCompileMethodDelegate; // stash it away so that it stays alive forever

            var ourCompileMethodPtr = EHNativeToManaged(
                Marshal.GetFunctionPointerForDelegate(ourCompileMethodDelegate),
                out n2mHookHelper
            );
        ...
        protected virtual unsafe void InvokeCompileMethodToPrepare(IntPtr method)
        {
            V21.CORINFO_METHOD_INFO methodInfo;
            byte* nativeStart;
            uint nativeSize;
            InvokeCompileMethodPtr.InvokeCompileMethod(
                method,
                IntPtr.Zero,
                IntPtr.Zero,
                &methodInfo,
        ...
        {
            var del = new JitHookDelegateHolder(
                this,
                InvokeCompileMethodPtr,
                compileMethod
            ).CompileMethodHook;
            return del;
        }
        ...
        private sealed class JitHookDelegateHolder
        {
            ...
            public JitHookDelegateHolder(
                Core21Runtime runtime,
            ...
            )
            {
                ...
                *pNativeSizeOfCode = 0;
                ...
                try
                {
                     ...
                     * -ade
                     */
                    var result = InvokeCompileMethodPtr.InvokeCompileMethod(
                        CompileMethodPtr,
                        jit,
                        corJitInfo,
                        methodInfo,
                    ...
                    {
                        MMDbgLog.Warning(
                            $"Native exception caught in JIT by exception helper (ex: 0x{nativeException:x16})"
                        );
                        return result;
                    }
                    ...
                    if (hookEntrancy == 1)
                    {
                        try
                        {
                            ...
                            {
                                genericClassArgs = new RuntimeTypeHandle[
                                    methodInfo->args.sigInst.classInstCount
                                ];
                                for (var i = 0; i < genericClassArgs.Length; i++)
                                {
                                    genericClassArgs[i] = JitHookHelpers
                                        .GetTypeFromNativeHandle(
                                            methodInfo->args.sigInst.classInst[i]
                                        )
                                        .TypeHandle;
                                }
                            }
                            ...
                            {
                                genericMethodArgs = new RuntimeTypeHandle[
                                    methodInfo->args.sigInst.methInstCount
                                ];
                                for (var i = 0; i < genericMethodArgs.Length; i++)
                                {
                                    genericMethodArgs[i] = JitHookHelpers
                                        .GetTypeFromNativeHandle(
                                            methodInfo->args.sigInst.methInst[i]
                                        )
                                        .TypeHandle;
                                }
                            }

                            var declaringType = JitHookHelpers
                                .GetDeclaringTypeOfMethodHandle(methodInfo->ftn)
                                .TypeHandle;
                            var method = JitHookHelpers.CreateHandleForHandlePointer(
                                methodInfo->ftn
                            );

                            // codeStart and codeStartRw are the same because this runtime doesn't distinguish them at this point in the JIT
                            Runtime.OnMethodCompiledCore(
                                declaringType,
                                method,
                                genericClassArgs,
                                genericMethodArgs,
        ...
        protected sealed class JitHookHelpersHolder
        {
            ...
            public delegate object CreateRuntimeMethodInfoStubD(
                IntPtr methodHandle,
                object loaderAllocator
            );
            public delegate RuntimeMethodHandle CreateRuntimeMethodHandleD(
                object runtimeMethodInfo
            );
            ...
            private readonly bool useModern;
            ...
            public RuntimeMethodHandle CreateHandleForHandlePointer(IntPtr handle) =>
                useModern
                    ? RuntimeMethodHandle.FromIntPtr(handle)
                    : CreateRuntimeMethodHandle!(
                        CreateRuntimeMethodInfoStub!(
                            handle,
                            MethodHandle_GetLoaderAllocator!(handle)
                        )
                    );

            public JitHookHelpersHolder(Core21Runtime runtime)
            {
                useModern = runtime.UseModernHandleResolution;
                const BindingFlags StaticNonPublic = BindingFlags.Static | BindingFlags.NonPublic;

                // Modern runtimes use RuntimeMethodHandle.FromIntPtr.
                if (!useModern)
                { // set up GetLoaderAllocator
                    var getLoaderAllocator = typeof(RuntimeMethodHandle).GetMethod(
                        "GetLoaderAllocator",
                        StaticNonPublic
                    );
                    ...
                    using (
                        var dmd = new DynamicMethodDefinition(
                            "MethodHandle_GetLoaderAllocator",
                            typeof(object),
                    ...
                    MethodHandle_GetLoaderAllocator =
                        invokeWrapper.CreateDelegate<MethodHandle_GetLoaderAllocatorD>();
                }

                if (useModern)
                {
                    // CORINFO_CLASS_HANDLE is a TypeHandle (MethodTable*); FromIntPtr accepts it.
                    GetTypeFromNativeHandle = handle =>
                ...
                { // set up GetTypeFromNativeHandle
                    ...
                    GetTypeFromNativeHandle =
                        getTypeFromHandleUnsafe.CreateDelegate<GetTypeFromNativeHandleD>();
                }

                if (useModern)
                {
                    // CORINFO_METHOD_HANDLE is a MethodDesc*; resolve via public APIs.
                    // Non-generic methods resolve here. Shared generics (GetMethodFromHandle
                    // throws) and global/module methods (null declaring type) cannot resolve;
                    // Both failures are limited to tracking and do not affect the detour.
                    GetDeclaringTypeOfMethodHandle = handle =>
                        MethodBase
                            .GetMethodFromHandle(RuntimeMethodHandle.FromIntPtr(handle))
                            ?.DeclaringType
                        ?? throw new InvalidOperationException(
                            "JIT method handle has no resolvable declaring type "
                                + "(global method or shared generic)."
                        );
                }
                else
                { // set up GetDeclaringTypeOfMethodHandle
                    var methodHandleInternal = typeof(RuntimeMethodHandle).Assembly.GetType(
                        "System.RuntimeMethodHandleInternal"
                    );
                    Helpers.DAssert(methodHandleInternal is not null);
                    var getDeclaringType = typeof(RuntimeMethodHandle).GetMethod(
                        "GetDeclaringType",
                        StaticNonPublic,
                        null,
                        new Type[] { methodHandleInternal },
                        null
                    );
                    ...
                    using (
                        var dmd = new DynamicMethodDefinition(
                            "GetDeclaringTypeOfMethodHandle",
                            typeof(Type),
                            new Type[] { typeof(IntPtr) }
                        )
                    )
                    ...
                    GetDeclaringTypeOfMethodHandle =
                        invokeWrapper.CreateDelegate<GetDeclaringTypeOfMethodHandleD>();
                }

                if (!useModern)
                { // set up CreateRuntimeMethodInfoStub
                    var runtimeMethodInfoStubCtorArgs = new[] { typeof(IntPtr), typeof(object) };
                    var runtimeMethodInfoStub = typeof(RuntimeMethodHandle).Assembly.GetType(
                        "System.RuntimeMethodInfoStub"
                    );
                    Helpers.DAssert(runtimeMethodInfoStub is not null);
                    var runtimeMethodInfoStubCtor = runtimeMethodInfoStub.GetConstructor(
                        runtimeMethodInfoStubCtorArgs
                    );
                    ...
                    CreateRuntimeMethodInfoStub =
                        runtimeMethodInfoStubCtorWrapper.CreateDelegate<CreateRuntimeMethodInfoStubD>();
                }

                if (!useModern)
                { // set up CreateRuntimeMethodHandle
                    var ctor = typeof(RuntimeMethodHandle)
                        .GetConstructors(
                    ...
                    CreateRuntimeMethodHandle =
                        ctorWrapper.CreateDelegate<CreateRuntimeMethodHandleD>();
                }
            }
        ...
        // .NET 10 requires the public FromIntPtr handle APIs.
        protected virtual bool UseModernHandleResolution => false;

        private static readonly FieldInfo RuntimeAssemblyPtrField = Type.GetType(
        ...
        {
            // The legacy object layout is unsafe when modern handle resolution is active.
            if (UseModernHandleResolution)
                throw new InvalidOperationException(
                    "MakeAssemblySystemAssembly is unreachable on modern runtimes (FromIntPtr handle resolution is used)."
                );
            ...
            var domOffset =
                ...
                + // PTR_PEFile                  m_pFile;
                IntPtr.Size
                + // PTR_PEFile                  m_pOriginalFile;
                IntPtr.Size
                + // PTR_Module                  m_pModule;
                sizeof(int)
                + // FileLoadLevel               m_level; // FileLoadLevel is an enum with unspecified type; I assume it defaults to 'int' because that's what `enum class` does
                IntPtr.Size
                ...
                + // DWORD                    m_notifyflags;
                sizeof(int)
                + // BOOL                        m_loading; // no matter the actual size of this BOOL, the next member is a pointer, and we'd always be misaligned
                IntPtr.Size
                ...
                sizeof(int)
                + // DWORD m_dwReasonForRejectingNativeImage;
            ...
            if (IntPtr.Size == 8)
            {
                domOffset += sizeof(int); // padding to align the next TADDR (which is a void*) (m_hExposedModuleObject)
            }
            ...
            var pAssemOffset =
                ...
                IntPtr.Size
                + // PTR_ClassLoader       m_pClassLoader;
                IntPtr.Size
                + // PTR_MethodDesc        m_pEntryPoint;
                IntPtr.Size
            ...
            var peAssemOffset =
                ...
                (
                    IsDebugClr
                        ? 0
                            ...
                            sizeof(int)
                            + // COUNT_T             m_size; // COUNT_T is a typedef of uint32_t
                            sizeof(int)
                            + // COUNT_T             m_allocation;
                            sizeof(int)
                            + // UINT32              m_flags;
                            //sizeof(int) + // padding to 8 bytes
                            IntPtr.Size
                            + // union { BYTE* m_buffer; WCHAR* m_asStr; };
                            sizeof(int)
                            + // int                 m_revision
                ...
                sizeof(int)
                + // BOOL                     m_MDImportIsRW_Debugger_Use_Only; // i'm pretty sure that these bools are sizeof(int)
                sizeof(int)
                + // Volatile<BOOL>           m_bHasPersistentMDImport;         // but they might not be, and it might vary (that would be a pain in the ass)
                IntPtr.Size
                ...
                sizeof(int)
                + // Volatile<LONG>           m_refCount; // fuck C long
