...
namespace MonoMod.Core.Platforms.Runtimes
{
    ...
    {
        public static CoreBaseRuntime CreateForVersion(
            Version version,
            ISystem system,
            IArchitecture arch
        )
            ...
            switch (version.Major)
            {
                ...
                case 3:
                    ...
                    return version.Minor switch
                    {
                        ...
                        _ => throw new PlatformNotSupportedException(
                            $"Unknown .NET Core 3.x minor version {version.Minor}"
                        ),
                    };
                ...
                default:
                    throw new PlatformNotSupportedException(
                        $"CoreCLR version {version} is not supported"
                    );
            }
        ...
        protected virtual string GetClrJitPath()
        {
            ...
            if (
                Switches.TryGetSwitchValue(Switches.JitPath, out var swValue)
                && swValue is string jitPath
            )
            {
                if (!IsMaybeClrJitPath(jitPath))
                {
                    MMDbgLog.Warning(
                        $"Provided value for MonoMod.JitPath switch '{jitPath}' does not look like a ClrJIT path"
                    );
                }
                else
                {
                    ...
                    {
                        MMDbgLog.Warning(
                            $"Provided path for MonoMod.JitPath switch was not loaded in this process. jitPath: {jitPath}"
                        );
                    }
                }
            }
        ...
        protected INativeExceptionHelper? NativeExceptionHelper =>
            lazyNativeExceptionHelper ??= System.NativeExceptionHelper;
