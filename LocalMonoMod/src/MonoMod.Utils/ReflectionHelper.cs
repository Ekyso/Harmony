...
namespace MonoMod.Utils
{
    public static partial class ReflectionHelper
    {
        ...
        public static Assembly Load(ModuleDefinition module)
        {
            ...
            {
                // On Mono/Android, Assembly.Load(byte[]) creates assemblies invisible to the
                // native JIT token resolver. Write directly to disk and use LoadFrom.
                var tempPath = Path.Combine(Path.GetTempPath(), $"MonoMod_DMD_{Guid.NewGuid():N}.dll");
                module.Write(tempPath);
                // DMD loaded via LoadFrom for Mono JIT token resolution
        ...
        public static Assembly Load(Stream stream)
        {
            ...
            if (PlatformDetection.Runtime == RuntimeKind.Mono)
            {
                ...
                // DMD loaded via LoadFrom for Mono JIT token resolution
                asm = Assembly.LoadFrom(tempPath);
            }
            else
            {
