    ...
    public sealed partial class DynamicMethodDefinition : IDisposable
    {
        ...
        internal static readonly bool _IsNewMonoSRE =
            ...
            && (
                ...
                // .NET 8 Mono uses _ilGenerator instead of il_info
                || typeof(DynamicMethod).GetField(
                    "_ilGenerator",
                    BindingFlags.NonPublic | BindingFlags.Instance
                ) != null
        ...
        // Cecil methods load into a separate assembly, so Mono uses DynamicMethod when SRE works.
        private static bool _PreferCecil =
            (
                PlatformDetection.Runtime is not RuntimeKind.Mono
                && (
                    // Check for stubbed-out SRE.
                    typeof(ILGenerator)
                        .Assembly.GetType("System.Reflection.Emit.DynamicILGenerator")
        ...
        internal static readonly ConstructorInfo c_IgnoresAccessChecksToAttribute =
            typeof(System.Runtime.CompilerServices.IgnoresAccessChecksToAttribute).GetConstructor(
                new[] { typeof(string) }
            )!;
        ...
        {
            var module = Module = ModuleDefinition.CreateModule(
                $"DMD:DynModule<{name}>?{GetHashCode()}",
        ...
        private void CreateFromDmd(
            DynamicMethodDefinition src,
            out ModuleDefinition Module,
            out MethodDefinition Definition
        )
        {
            var module = Module = ModuleDefinition.CreateModule(
                $"DMD:DynModule<{src.Name}>?{GetHashCode()}",
                new ModuleParameters()
                {
                    Kind = ModuleKind.Dll,
                    ReflectionImporterProvider = MMReflectionImporter.ProviderNoDefault,
        ...
        )
        {
            ...
            _CreateDynModule(
                orig.GetID(simple: true),
                (orig as MethodInfo)?.ReturnType,
                argTypes,
                out Module,
                out def
            );
        ...
        public MethodInfo Generate() => Generate(null);

        public MethodInfo Generate(object? context)
        {
            var dmdType = Switches.TryGetSwitchValue(Switches.DMDType, out var swValue)
                ? swValue as string
                : null;
            ...
            {
                if (
                    dmdType.Equals("dynamicmethod", StringComparison.OrdinalIgnoreCase)
                ...
                if (
                    dmdType.Equals("cecil", StringComparison.OrdinalIgnoreCase)
...
#if NETFRAMEWORK
                if (
                    dmdType.Equals("methodbuilder", StringComparison.OrdinalIgnoreCase)
            ...
            if (dmdType is not null)
            {
                ...
                if (type != null)
                {
                    ...
                    var gen = _DMDGeneratorCache.GetOrAdd(
                        dmdType,
                        _ => (IDMDGenerator)Activator.CreateInstance(type)!
                    );
...
#if NETFRAMEWORK
            if (
                Definition!.Body.ExceptionHandlers.Any(eh =>
