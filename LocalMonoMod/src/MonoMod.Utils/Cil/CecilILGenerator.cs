...
namespace MonoMod.Utils.Cil
{
    ...
    public sealed class CecilILGenerator : ILGeneratorShim
    {
        ...
        private static readonly ConstructorInfo c_LocalBuilder = t_LocalBuilder
            .GetConstructors(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance)
            .OrderByDescending(c => c.GetParameters().Length)
            .First();
        private static readonly FieldInfo? f_LocalBuilder_position = t_LocalBuilder.GetField(
            "position",
            BindingFlags.NonPublic | BindingFlags.Instance
        );
        private static readonly FieldInfo? f_LocalBuilder_is_pinned = t_LocalBuilder.GetField(
            "is_pinned",
            BindingFlags.NonPublic | BindingFlags.Instance
        );
        ...
        private static readonly Dictionary<short, OpCode> _MCCOpCodes =
        ...
        private readonly Dictionary<Label, LabelInfo> _LabelInfos =
            new Dictionary<Label, LabelInfo>();
        private readonly List<LabelInfo> _LabelsToMark = new List<LabelInfo>();
        private readonly List<LabelledExceptionHandler> _ExceptionHandlersToMark =
            new List<LabelledExceptionHandler>();

        private readonly Dictionary<LocalBuilder, VariableDefinition> _Variables =
            new Dictionary<LocalBuilder, VariableDefinition>();

        private readonly Stack<ExceptionHandlerChain> _ExceptionHandlers =
            new Stack<ExceptionHandlerChain>();
        ...
        {
            var mref = IL.Body.Method.Module.ImportReference(info);
            return mref;
        }
            ...
            if (_ExceptionHandlersToMark.Count != 0)
            {
                foreach (var exHandler in _ExceptionHandlersToMark)
                    IL.Body.ExceptionHandlers.Add(
                        new ExceptionHandler(exHandler.HandlerType)
                        {
        ...
        public override LocalBuilder DeclareLocal(Type localType, bool pinned)
        {
            ...
            var handle = (LocalBuilder)(
                c_LocalBuilder_params == 4
                    ? c_LocalBuilder.Invoke(new object?[] { index, localType, null, pinned })
                : c_LocalBuilder_params == 3
                    ? c_LocalBuilder.Invoke(new object?[] { index, localType, null })
                : c_LocalBuilder_params == 2
                    ? c_LocalBuilder.Invoke(new object?[] { localType, null })
        ...
        {
            if (
                opcode.OperandType == SRE.OperandType.ShortInlineVar
        ...
        public override void Emit(SRE.OpCode opcode, long arg) =>
            Emit(IL.Create(CecilILGenerator._(opcode), arg));

        public override void Emit(SRE.OpCode opcode, float arg) =>
            Emit(IL.Create(CecilILGenerator._(opcode), arg));

        public override void Emit(SRE.OpCode opcode, double arg) =>
            Emit(IL.Create(CecilILGenerator._(opcode), arg));

        public override void Emit(SRE.OpCode opcode, string str) =>
            Emit(IL.Create(CecilILGenerator._(opcode), str));

        public override void Emit(SRE.OpCode opcode, Type cls) =>
            Emit(IL.Create(CecilILGenerator._(opcode), _(cls)));

        public override void Emit(SRE.OpCode opcode, FieldInfo field) =>
            Emit(IL.Create(CecilILGenerator._(opcode), _(field)));

        public override void Emit(SRE.OpCode opcode, ConstructorInfo con) =>
            Emit(IL.Create(CecilILGenerator._(opcode), _(con)));

        public override void Emit(SRE.OpCode opcode, MethodInfo meth) =>
            Emit(IL.Create(CecilILGenerator._(opcode), _(meth)));
        ...
        public override void Emit(SRE.OpCode opcode, Label[] labels)
        {
            ...
            var ins = IL.Create(
                CecilILGenerator._(opcode),
                labelInfos.Select(labelInfo => labelInfo!.Instruction).ToArray()
            );
        ...
        public override void Emit(SRE.OpCode opcode, LocalBuilder local) =>
            Emit(IL.Create(CecilILGenerator._(opcode), _(local)));

        public override void Emit(SRE.OpCode opcode, SignatureHelper signature) =>
            Emit(
                IL.Create(
                    CecilILGenerator._(opcode),
                    IL.Body.Method.Module.ImportCallSite(signature)
                )
            );

        public void Emit(SRE.OpCode opcode, ICallSiteGenerator signature) =>
            Emit(
                IL.Create(
                    CecilILGenerator._(opcode),
                    IL.Body.Method.Module.ImportCallSite(signature)
        ...
        public override void EmitWriteLine(FieldInfo fld)
        {
            ...
            Emit(
                IL.Create(
                    OpCodes.Call,
                    _(typeof(Console).GetMethod("WriteLine", new[] { fld.FieldType })!)
                )
            );
        }
        ...
        public override void EmitWriteLine(string value)
        {
            ...
            Emit(
                IL.Create(
                    OpCodes.Call,
                    _(typeof(Console).GetMethod("WriteLine", new[] { typeof(string) })!)
                )
            );
        }
            ...
            public void EndHandler(LabelledExceptionHandler handler)
            {
                ...
                switch (handler.HandlerType)
                {
                    ...
                    case ExceptionHandlerType.Fault:
                        // endfault and endfinally are the same opcode (0xDC); the runtime distinguishes
                        // them by the enclosing clause type, which is already ExceptionHandlerType.Fault.
                        IL.Emit(SRE.OpCodes.Endfinally);
                        break;
                ...
                EndHandler(
                    _Handler
                        ?? throw new InvalidOperationException(
                            "Cannot end when there is no current handler!"
                        )
                );
