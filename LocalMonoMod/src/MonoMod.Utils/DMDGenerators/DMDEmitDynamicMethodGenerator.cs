...
namespace MonoMod.Utils
{
    public sealed class DMDEmitDynamicMethodGenerator : DMDGenerator<DMDEmitDynamicMethodGenerator>
    {
        ...
        protected override MethodInfo GenerateCore(DynamicMethodDefinition dmd, object? context)
        {
            ...
            var name =
                dmd.Name
                ?? DebugFormatter.Format($"DMD<{(object?)orig ?? def.GetID(simple: true)}>");
            var retType = (orig as MethodInfo)?.ReturnType ?? def.ReturnType.ResolveReflection();

            MMDbgLog.Trace(
                $"new DynamicMethod: {retType} {name}({string.Join(",", argTypes.Select(type => type?.ToString()).ToArray())})"
            );
            if (orig != null)
                MMDbgLog.Trace($"orig: {orig}");
            MMDbgLog.Trace(
                $"mdef: {def.ReturnType?.ToString() ?? "NULL"} {name}({string.Join(",", def.Parameters.Select(arg => arg?.ParameterType?.ToString() ?? "NULL").ToArray())})"
            );

            // StackCrawlMark uses the owner module to resolve unqualified type names.
            var ownerType =
                orig?.DeclaringType ?? context as Type ?? typeof(DynamicMethodDefinition);
