...
{
    /// <summary>A DMDGenerator implementation using Mono.Cecil.</summary>
...
#pragma warning disable CA2000 // Dispose objects before losing scope
                    ...
                    module = ModuleDefinition.CreateModule(
                        name,
...
#pragma warning restore CA2000 // Dispose objects before losing scope
                    ...
                    module.Assembly.CustomAttributes.Add(
                        new CustomAttribute(
                            module.ImportReference(
                                DynamicMethodDefinition.c_UnverifiableCodeAttribute
                            )
                        )
                    );
                    ...
                    if (dmd.Debug)
                    {
                        var caDebug = new CustomAttribute(
                            module.ImportReference(DynamicMethodDefinition.c_DebuggableAttribute)
                        );
                        caDebug.ConstructorArguments.Add(
                ...
                var tr_IsVolatile = new TypeReference(
...
#pragma warning restore IDE0039 // Use local function
                ...
                for (var instri = 0; instri < body.Instructions.Count; instri++)
                {
                    ...
                    if (accessChecksIgnored != null && operand is MemberReference mref)
                    {
                        ...
                        {
                            var caAccess = new CustomAttribute(
                                module.ImportReference(
                                    DynamicMethodDefinition.c_IgnoresAccessChecksToAttribute
                                )
                            );
                            caAccess.ConstructorArguments.Add(
                                new CustomAttributeArgument(
                                    module.ImportReference(
                ...
                var envDmdDump = Switches.TryGetSwitchValue(Switches.DMDDumpTo, out var dumpToVal)
                    ? dumpToVal as string
                    : null;
                ...
                return asm.GetType(
                                typeDef.FullName.Replace("+", "\\+", StringComparison.Ordinal),
                                false,
                                false
                            )!
                        .GetMethod(
